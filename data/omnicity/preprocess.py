...
def polar_transform_sate_image(sate_img_path):
    global shared_polar_sate_img_dir
    ...
    img_name = osp.basename(sate_img_path)
    sate_img = cv2.imread(sate_img_path)
    polar_pano_img = sample_bilinear(sate_img, x, y).astype(np.uint8)
    polar_pano_img_path = osp.join(shared_polar_sate_img_dir, img_name)
    cv2.imwrite(polar_pano_img_path, polar_pano_img)


def rotate_pano_image(pano_img_path):
    global shared_rotated_pano_img_dir

    img_name = osp.basename(pano_img_path)
    ratio = int(img_name.split('_')[-2][1:])
    pano_img = cv2.imread(pano_img_path)
    beg_idx = int(ratio / 360 * pano_img.shape[1])
    rotated_pano_img = np.concatenate([pano_img[:, beg_idx:, :], pano_img[:, :beg_idx, :]], axis=1)
    rotated_pano_img_path = osp.join(shared_rotated_pano_img_dir, img_name)
    cv2.imwrite(rotated_pano_img_path, rotated_pano_img)
...
def geo_transform_height_image(height_img_path):
    global shared_geo_height_img_dir
    ...
    geo_height_img_path = osp.join(shared_geo_height_img_dir, osp.splitext(img_name)[0] + ".jpg")
    cv2.imwrite(geo_height_img_path, mask.astype(np.uint8))


def geo_transform_image(height_img_path, semantic_img_path):
...
def main(input_dir, output_dir, data_type):
    if data_type == 'sate':
        global shared_polar_sate_img_dir
        shared_polar_sate_img_dir = output_dir
        process_func = polar_transform_sate_image
    elif data_type == 'pano':
        global shared_rotated_pano_img_dir
        shared_rotated_pano_img_dir = output_dir
        process_func = rotate_pano_image
    elif data_type == 'geo':
        global shared_geo_height_img_dir
        shared_geo_height_img_dir = output_dir
        process_func = geo_transform_height_image
    elif data_type == 'geo_semantic':
        global shared_geo_semantic_img_dir
        shared_geo_semantic_img_dir = output_dir
        process_func = geo_transform_image

    to_process_paths = [osp.join(input_dir, _) for _ in os.listdir(input_dir)]
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        print(f"Usage: python {script_name} [input_dir] [output_dir] [data_type]")
        sys.exit(0)

    input_dir = sys.argv[1].strip()
    output_dir = sys.argv[2].strip()
    data_type = sys.argv[3].strip()
    if not osp.exists(output_dir):
        os.mkdir(output_dir)

    main(input_dir, output_dir, data_type)


# python data/omnicity/preprocess.py \
#     /mnt/petrelfs/share_data/bigdata_rs/datasets/OmniCity/OmniCity/OmniCity-dataset-trainval/street-level/image-panorama/image-panorama-test/ \
#     /mnt/petrelfs/share_data/zhonghuaping.p/datasets/OmniCity/rotated-image-panorama/test \
#     pano

# python data/omnicity/preprocess.py \
#     /mnt/petrelfs/share_data/bigdata_rs/datasets/OmniCity/OmniCity/OmniCity-dataset-trainval/satellite-level/image-satellite/satellite-image-view1/view1-train/ \
#     /mnt/petrelfs/share_data/zhonghuaping.p/datasets/OmniCity/medium-view/train/polar-image-satellite \
#     sate
...
#     /mnt/petrelfs/share_data/zhonghuaping.p/datasets/OmniCity/geo-height-panorama/test \
#     geo